...
buildah run ${c} -- apt install --yes --quiet=2 gnupg python3 python3-venv git sqlite3
...
buildah run ${c} -- apt update --quiet=2
buildah run ${c} -- apt install --yes --quiet=2 pdns-server pdns-backend-sqlite3
buildah run ${c} -- apt purge --yes --quiet=2 pdns-backend-bind
